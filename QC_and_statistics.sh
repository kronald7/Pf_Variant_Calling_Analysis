#!/bin/bash
#SBATCH --job-name="Fastq_bam_qc"
#SBATCH --time=100:00:00
#SBATCH --ntasks=24
#SBATCH --mem=80G
#SBATCH --mail-user=ronky@email.unc.edu
...
usage() { echo -e "Usage: $0 [-i <input list>] [-f <fastq directory>] [-r <reference directory>] [-b <bam directory>] [-u <unpaired directory>] [-s <stat directory>] [-k <kit name>]" 2>&1
 echo -e "-i : list of sample IDs in a text file "
 echo -e "-f: full path to the fastq files"
 echo -e "-r: full path to reference files"
...
fastq_dir='/work/users/r/o/ronky/reseq_fastqs'
ref_dir='/proj/ideel/julianog/users/Kronald/reference/Data_WGS_UNC'
bam_dir='/work/users/r/o/ronky/bam/'
unpaired_dir='/work/users/r/o/ronky/unpaired'
stat_dir='/work/users/r/o/ronky/stats'
lib_kit='/nas/longleaf/apps/trimmomatic/0.36/Trimmomatic-0.36/adapters/TruSeq3-PE.fa'

module load trimmomatic/0.36 bwa/0.7.15 gatk samtools datamash/1.3 r/4.1.0
...
for i in $(cat $input)
  do
   cd $fastq_dir
   trimmomatic PE "$i"_1.fastq.gz "$i"_2.fastq.gz "$i"_R1_paired.fq.gz "$i"_R1_unpaired.fq.gz "$i"_R2_paired.fq.gz "$i"_R2_unpaired.fq.gz  ILLUMINACLIP:$lib_kit:2:30:10 LEADING:3 TRAILING:3 MINLEN:3  SLIDINGWINDOW:5:20 -threads 12
    mv *unpaired.fq.gz $unpaired_dir
    cd $bam_dir
    bwa mem -t 16 -M -R "@RG\tID:'$i'\tLB:'$i'\tPL:illumina\tSM:'$i'\tPU:'$i'" $ref_dir/Pf3D7_human.fa $fastq_dir/"$i"_R1_paired.fq.gz $fastq_dir/"$i"_R2_paired.fq.gz > "$i".sam
    ...
    samtools view -b -h "$i".sorted.dup.bam -T $ref_dir/human.fa -L $ref_dir/human.bed > "$i".sorted.dup.hs.bam
...
cd $stat_dir
cat *_insert2.txt > InsertSize_Final.txt
rm *_insert2.txt
